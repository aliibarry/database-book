...
## Species notes _(optional)_

If your data contains different species, you will also need datasets that allow you map gene symbols/IDs between species. To do this, you will need the `biomaRt` library. An example of mapping gene symbols between mouse and human is shown below: 
...
geneV1 = getLDS(attributes = c("mgi_symbol","ensembl_gene_id"), filters = "ensembl_gene_id", values = count$X, mart = mouse, attributesL = c("hgnc_symbol", "ensembl_gene_id", "description"), martL = human, uniqueRows=T)
...
If you plan to store data this way, move to the *User Interface* section now. *NOTE*, some functions will need to be modified to read from a `.RData` instead. If you are working with large amounts of data, you may find it useful to set up an SQL database (below).

## Building an SQL database (recommended)
...
There are several advantages of using SQL databases over storing data in `.RData` files:

1. Scalability: SQL databases are designed to handle large amounts of data, making it easy to scale up as your data grows. In contrast, .RData files can become unwieldy and slow to load as the size of your data increases.
2. Querying: SQL databases provide a powerful and flexible language (SQL) for querying and analyzing data. This allows you to easily extract and manipulate subsets of your data based on specific criteria. In contrast, querying data in .RData files requires writing custom R code, which can be time-consuming and error-prone.
3. Security: SQL databases provide a range of security features to protect your data, such as authentication, authorization, and encryption. This can be especially important if you are dealing with sensitive data, such as personal information or financial data. .RData files, on the other hand, are generally less secure and can be easily accessed by anyone with access to your computer or file system.
4. Collaboration: SQL databases are designed to facilitate collaboration among multiple users or teams. With SQL databases, multiple users can access and manipulate the same data simultaneously, while ensuring that changes are tracked and versioned. In contrast, sharing .RData files can be difficult, especially if multiple users need to make changes to the same file.

An RSQLite `.db` object is a database connection to a SQLite database file. The structure of a SQLite database consists of one or more tables, which are composed of rows and columns. The following functions will be useful in creating and updating the `.db` object. 
