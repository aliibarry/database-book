...
2. Compile all data into an `.RData` or `.db` object using `setup.R`.
...
Here, we provide an overview for bulk count data (provided as `setup.R`). `Seurat` objects (for sc/snRNA-seq or ATAC-seq) are read in separately in our example.
